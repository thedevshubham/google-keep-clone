...
import { ReactComponent as LabelIcon } from "../../images/label-svgrepo-com.svg";
...
const sideBarList = [
  { type: "notes", name: "Notes", link: "/", image: <NotesIcon /> },
  {
    type: "reminders",
    name: "Reminders",
    link: "/reminders",
...
const SideMenu = () => {
  ...
  const handleSideLink = (link='', title='') => {
    ...
    console.log(path);
    dispatch({ type: SET_SIDE_LINK_DATA, key: "activeLink", value: link || window.location.pathname });
    dispatch({ type: SET_SIDE_LINK_DATA, key: "title", value: decodeURI(title || window.location.pathname).replace('/', '') });
  }
    ...
    <div className="sideMenu">
      <div className="sideMenu__container">
        {sideBarList.map((item) => {
          return (
            <Link to={item.name} className="menuItem" key={item.link} onClick={() => handleSideLink(item.link, item.name)}>
              <div className={`sideMenu__block ${sideLinkData.activeLink === item.link ? 'activeLink' : ''}`}>
                {item.image}
                {!isDrawerClicked && (
